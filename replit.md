...
- **Phase 31 — Ops Kanban Board:**
  - `artifacts/kenyavet/src/pages/ops.tsx` — added `LayoutGrid`, `Play` to lucide imports; added `KANBAN_COLUMNS` const (4 columns: Awaiting Payment/amber, In Progress/blue, Completed/emerald, Cancelled/slate); added `KanbanCard` component: worker name + package pill, role, employer name + neighbourhood, SLA badge (overdue/urgent/ok), trust score pill when complete, "Start Vetting →" action button (with `Play` icon) for `pending_payment` cards that calls `onStartVetting(id)` with `e.stopPropagation()`, date footer; added `KanbanBoard` component: `grid-cols-2 xl:grid-cols-4` layout, each column has a color-coded header with dot + label + count badge, min-h-400px body, "No requests" empty state; in `Ops` component: changed `activeView` type from `"queue" | "analytics"` to `"queue" | "analytics" | "kanban"`; modified `load` useCallback to use `?status=all` when `activeView === "kanban"`, added `activeView` to deps array; changed `useEffect` trigger from `activeView === "queue"` to `activeView !== "analytics"` so kanban also auto-loads; added `startVetting` useCallback: calls `PATCH /admin/requests/:id/status` with `{ status: "in_progress" }`, toasts success/failure, re-calls `load()`; added "Board" toggle button (LayoutGrid icon) to the view toggle between Queue and Analytics; Refresh button now shows for both `queue` and `kanban` views; added kanban render branch in main content: shows spinner while loading, then `<KanbanBoard requests={requests} onSelect={setSelected} onStartVetting={startVetting} />`; clicking any kanban card opens the existing OpsDrawer; the drawer's `onUpdated` callback calls `load()` so board refreshes after changes
- **Phase 30 — Auto-Message Notifications:**
  - `artifacts/api-server/src/routes/documents.ts` — added `messagesTable`, `usersTable` to `@workspace/db` import; after `INSERT INTO documents`, auto-inserts a message in the request thread (non-blocking, wrapped in try/catch so doc upload succeeds regardless): looks up employer name for employer uploads; message body: `📎 Document uploaded: [label: ][fileName]` (employer) or `📎 KenyaVet has added a document: [label: ][fileName]` (ops/admin); senderName: actual employer name or "KenyaVet Team"
  - `artifacts/api-server/src/routes/admin.ts` — added `messagesTable` to `@workspace/db` import; after `PATCH /admin/steps/:stepId` updates step to `status === "completed"`, fetches the vetting request to get `workerName` and auto-inserts a message (non-blocking try/catch); message body: `✅ [stepName] has been completed for [workerName]'s vetting. Notes: [notes (120 char truncated)]`; senderName: "KenyaVet Ops" or "KenyaVet Admin" based on role; both auto-messages appear immediately in the employer's Messages thread and contribute to the unread count badge
- **Phase 29 — Per-Request Activity Timeline:**
  - `artifacts/api-server/src/routes/vetting-requests.ts` — added `documentsTable`, `messagesTable`, `reportsTable` to `@workspace/db` import; added `asc` to drizzle-orm import; new `GET /vetting-requests/:id/timeline` endpoint (requireAuth, employer-scoped): fetches all event sources in parallel (vettingStepsTable, documentsTable, messagesTable, reportsTable); builds flat events array with types: `submitted` (request.createdAt, worker name/role), `payment_confirmed` (first step createdAt as payment proxy, M-Pesa ref), `step_completed` (step.completedAt, optional notes snippet), `document_uploaded` (doc.createdAt, label + fileName, employer vs KenyaVet source), `message` (message.createdAt, sender label + 90-char body snippet), `report_published` (report.createdAt, trust score), `cancelled` (request.updatedAt); sorts all events ascending by `at`; returns `{ events: [...] }`
  - `artifacts/kenyavet/src/pages/vetting-request-detail.tsx` — added `History`, `Award`, `CreditCard` to lucide imports; added `TimelineEvent` interface (`type`, `title`, `detail`, `at`); added `timeline` state; added `loadTimeline()` async function (fetches `/vetting-requests/:id/timeline`, silently ignores errors); wired into `useEffect` alongside `loadMessages()` and `loadDocuments()`; Timeline card added to sidebar between Package Details and action buttons — shows only when `timeline.length > 0`; card has a `History` icon header; vertical connector line (absolute left-px bg-border) links all event rows; each row has a coloured icon circle (blue=submitted, emerald=payment, teal=step, violet=document, sky=message, amber=report, red=cancelled), event title, relative timestamp ("Xm/h/d ago" or "D Mon"), and optional detail line with `line-clamp-2`; icon and colour selected via inline `iconMap`/`colorMap` objects keyed on event type
- **Phase 28 — Ops Document Visibility + Upload:**
  - `artifacts/kenyavet/src/pages/ops.tsx` — added `useRef` to React import; `API_BASE` to `@/lib/api` import; `Paperclip`, `Upload`, `Download`, `File`, `Trash2` to lucide imports; `DocRecord` interface (same shape as vetting-request-detail); `formatFileSize(bytes)` helper; `DOC_LABELS` constant array (DCI Certificate, ID Scan, Reference Letter, Police Clearance, Address Verification, Social Media Report, Other); `OpsDocumentsPanel` component: fetches `GET /vetting-requests/:id/documents` on mount, label `<select>` with preset options, Upload button triggers hidden file `<input>`, same 3-step upload flow (presigned URL → GCS PUT → POST metadata), file list with filename, label pill, "KenyaVet" (emerald) vs "Employer" (blue) source badge, file size, Download link, Delete button with spinner; empty state with placeholder text; wired into `OpsDrawer` between Ops Notes and Report Builder sections
- **Phase 27 — Document Upload:**
  - **Object Storage provisioned** — GCS bucket created via `setupObjectStorage()`; secrets `DEFAULT_OBJECT_STORAGE_BUCKET_ID`, `PUBLIC_OBJECT_SEARCH_PATHS`, `PRIVATE_OBJECT_DIR` added to environment
  - `lib/db/src/schema/documents.ts` — new `documents` table: id, requestId (FK→vetting_requests, cascade delete), uploadedBy (FK→users), uploaderRole (employer/ops/admin), fileName, fileSize (bytes), mimeType, objectPath (e.g. `/objects/uploads/<uuid>`), label (optional), createdAt; exported from schema index; DB pushed
  - `lib/object-storage-web/` — copied from skill template; `tsconfig.json` updated with `composite: true`, `declarationMap: true`, `emitDeclarationOnly: true` for root tsconfig reference; added to root `tsconfig.json` references and `artifacts/kenyavet/tsconfig.json` references; added as `@workspace/object-storage-web: workspace:*` devDependency to kenyavet
  - `artifacts/api-server/src/lib/objectStorage.ts` + `objectAcl.ts` — copied from skill template; `@google-cloud/storage` and `google-auth-library` added as server dependencies; `zod` added as dependency (for storage route inline schemas)
  - `artifacts/api-server/src/routes/storage.ts` — copied from skill template; removed `@workspace/api-zod` import; added inline Zod schemas for `RequestUploadUrlBody` and `RequestUploadUrlResponse`; serves presigned upload URLs at `POST /storage/uploads/request-url` and streams private objects at `GET /storage/objects/*`; mounted in routes/index.ts
  - `artifacts/api-server/src/routes/documents.ts` — new route file; `GET /vetting-requests/:id/documents` (requireAuth): employer-scoped; returns chronological list; `POST /vetting-requests/:id/documents` (requireAuth): registers a document after client-side GCS upload; accepts `{fileName, fileSize, mimeType, objectPath, label?}`; `DELETE /vetting-requests/:id/documents/:docId` (requireAuth): employer can only delete own uploads, ops/admin can delete any; mounted in routes/index.ts
  - `package.json` — added `pnpm.overrides` for `react` and `react-dom` at `19.1.0` to prevent Uppy v5 from pulling a duplicate React
  - `artifacts/kenyavet/src/pages/vetting-request-detail.tsx` — added `DocRecord` interface; `documents`, `docUploading`, `deletingDocId` state; `docInputRef` ref; `loadDocuments()` called in useEffect alongside `loadMessages()`; `handleDocUpload(e)`: 3-step flow — fetch presigned URL with auth, PUT file to GCS presigned URL, POST metadata to `/documents`; `handleDocDelete(docId)`: DELETE with confirmation via disabled state; Documents card added after Messages section (inside `lg:col-span-2` column) — shows file name, size, upload date, KenyaVet badge for ops-uploaded docs, Download link (`${API_BASE}/storage${doc.objectPath}`), Delete button; hidden file `<input>` triggered by Upload button; empty state with instructions; Paperclip, Upload, Download, File icons imported from lucide-react; `API_BASE` imported from `@/lib/api`
- **Phase 26 — Messages Inbox:**
  - `artifacts/api-server/src/routes/messages.ts` — `GET /messages/threads` (requireAuth): fetches all messages joined with vettingRequests and users in one query; groups by requestId in JS; computes `unreadCount` per thread (messages from opposite role where isRead=false), `lastMessageBody/Sender/Role/At`, `totalMessages`; employer sees only their own request threads, ops/admin see all; results sorted by latest message descending
  - `artifacts/kenyavet/src/pages/messages.tsx` — new page; two-panel layout (thread list left, chat panel right); thread list shows worker name, role, request status badge, employer name (ops/admin only), last sender + message preview, relative timestamp, unread count pill; clicking a thread loads messages via `GET /vetting-requests/:id/messages`, auto-marks them as read, updates local thread unread count; chat panel shows bubble-style messages (own right/primary, other left/muted), sender name above first bubble of each group, relative timestamp below each; compose textarea with Enter-to-send (Shift+Enter for newline), Send button; "View Request" link to `/vetting-requests/:id`; mobile: stacked panels with back button via `showThread` state; empty state in both panels
  - `artifacts/kenyavet/src/components/layout/AppLayout.tsx` — "Messages" nav item added (MessageSquare icon) between Vetting Requests and My Staff; unread badge moved from `/vetting-requests` to `/messages`; simplified active chevron logic (hidden when badge is showing); MessageSquare imported from lucide-react
  - `artifacts/kenyavet/src/App.tsx` — `import Messages from "@/pages/messages"`; `<Route path="/messages">` registered after Analytics route
